...
{{-- Delete Users Modal --}}
<div class="modal fade" id="deleteUsersModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
...
<div id="offcanvaseditUsersModal" aria-labelledby="offcanvasWithBothOptionsLabel"
        ...
        <div class="container-fluid">
            <form id="updateUser" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    ...
                    <input type = "text" class="form-control" placeholder="Full Name" id="edit_full_name" name="full_name" required>

                    <label for="" class="form-label mt-2">Username</label>
                    <input type = "text" class="form-control" placeholder="Username" id="edit_full_username" name="username" required>

                    <label for="" class="form-label mt-2">Email:</label>
                    <input type = "email" class="form-control" placeholder="Email" id="edit_full_email" name="email" required>
                    ...
                    <input type="password" class="form-control" placeholder="Confirm Password" name="confirm_password" id="edit_full_confirm_password">
                ...
                <button type="button" class="btn btn-secondary mt-2 float-end" data-bs-dismiss="offcanvas">Close</button>
                ...
                success: function (response) {
                    ...
                    $.each(response.users, function (key, item) {
                        const tr = $('<tr><td class="d-none">' + item.id + '</td>' +
                            '<td><img src="{{ url('/assets/users') }}/' + item.image + '" width="100px" height="100px" alt="image" class="rounded-circle">' + '</td>' +
                            '<td>'+item.name+'</td>' +
                            '<td>'+item.username+'</td>' +
                            '<td>'+item.email+'</td>' +
                ...
                success: function (response) {
                    if(response.status === 400)
                    {
                        ...
                        $.each(response.errors, function (key, error_value) {
                            $('#update_errorlist').append('<li>'+error_value+'</li>');
                        });
                    }
