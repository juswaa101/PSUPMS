        ...
        $(document).ready(function() {
            ...
            function filterByNameAndDate() {
                ...
                $.ajax({
                    ...
                    url: '/head/reports/fetch/' + $('#projects option:selected').val(),
                    dataType: 'json',
                    success: function(data) {
            ...
            $('#projects').on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $.each(data.project, function(key, item) {
                            let tableName = $('#reportTable').DataTable();
                            // let oldNode = tableName.$("td:contains('"+item.report_id+"')").parent('tr');
                ...
                if ($('#projects').val() != null) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '/head/reports/fetch/project/' + $('#projects').val(),
                        dataType: 'json',
                        success: function(data) {
            ...
            $('#download').on('click', function() {
                ...
                if ($('#reportTable').DataTable().rows().count() !== 0) {
                    $.ajax({
                        type: 'get',
                        url: '/head/reports/generate-report/' + $('#projects').val(),
                        data: {
                            name: name,
                            date: date,
                            quarter: quarter
